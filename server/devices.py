...
from utils import *
from server.infrastructure.device.devicetypes import DEVICE_TYPES,\
    validate_device_type
from server.infrastructure.device.services import SERVICES, validate_services
from server.infrastructure.device.properties import PROPERTIES
...
class Device(Resource):
    ...
    def get_info(self):
        return {"device_id": self.id, "name": self.name, "address": self.address,\
                "type": self.device_type.type.name, "services": self.services_ids,\
...
class DevicesList(Resource):
    ...
    def add_device(self, device):
        ...
        if existing_dev is not None:
            ...
            #deve ir buscar o ID ao gateway para ter ids globais
            #comm = Communicator("224.0.1.187")
            device_id = self.server.id_gen.new_device_id()
        
        #alterar a criacao do Device pondo todos os campos
        res = Device(self, device_id, name=device["name"], address=device["address"], type_id=device["type"], services=device["services"])
    ...
    def check_existing_device(self, device_address):
        if not validate_IPv4(device_address):
            raise AppError(defines.Codes.BAD_REQUEST, "Invalid IP address ("+str(device_address)+")") 
    ...
    def change_state(self, new_state):
        ...
        for p in new_state.keys():
            if not str(p) in keys:
                raise AppError(defines.Codes.BAD_REQUEST, "Device doesn't have property '"+str(p)+"'")
        
        for k in new_state.keys():
            if isinstance(k, int):
                for p in self.state:
                    if p["property_id"] == int(k):
                        prop = PROPERTIES[int(k)]
                        if prop.validate(new_state[k]):
                            if prop.accessmode in ["WO", "RW"]:
                                ...
                                raise AppError(defines.Codes.FORBIDDEN, "Property ("+str(k)+") can't be written (access mode: "+str(prop.accessmode))
                        else:
                            raise AppError(defines.Codes.BAD_REQUEST, "Invalid property new value ("+str(new_state[k])+")")    
                ...
                try:
                    ...
                    for p in self.state:
                        if p["name"] == str(k):
                            prop = PROPERTIES[int(p["property_id"])]
                            if prop.validate(new_state[k]):
                                if prop.accessmode in ["WO", "RW"]:
                                    if prop.valuetype_class == valuetypes.SCALAR:
                                        p["value"] = int(new_state[k])
                    ...
                    if p["property_id"] == key:
                        ...
                        if prop.validate(new_state[k]):
                            if prop.accessmode in ["WO", "RW"]:
                                ...
                                raise AppError(defines.Codes.FORBIDDEN, "Property ("+str(key)+") can't be written (access mode: "+str(prop.accessmode))
                        else:
                            raise AppError(defines.Codes.BAD_REQUEST, "Invalid property new value ("+str(new_state[k])+")")
                        
            else:
                raise AppError(defines.Codes.INTERNAL_SERVER_ERROR, "Property id or name invalid format. Must be int or string.")   
    ...
    def render_PUT(self, request):
        if(request.content_type is defines.Content_types.get("application/json")):
            try:
                ...
                print "ERROR: Request payload not json"
                return error(defines.Codes.BAD_REQUEST, "Request payload not properly formated json")
              
            try:
                if isinstance(body, dict):
                    self.change_state(body)
                else:
                    raise AppError(defines.Codes.BAD_REQUEST, "Content must be a json dictionary") 
            ...
            return error(defines.Codes.UNSUPPORTED_CONTENT_FORMAT, "Request content must be application/json")
        ...
        if(request.content_type is defines.Content_types.get("application/json")):
            ...
            try:
                if isinstance(body, list):
                    ...
                    return error(defines.Codes.BAD_REQUEST, "Request content must be json list of services or a json dictionary with only one service")
                
                self.payload = self.get_payload()
                return status(defines.Codes.CHANGED, self.payload)
            
            except:
                return error(defines.Codes.BAD_REQUEST, "Request content must specify a service id in json format")
        else:
            return error(defines.Codes.UNSUPPORTED_CONTENT_FORMAT, "Request content format not application/json") 
    ...
    def render_DELETE(self, request):
        try:
            ...
            try:
                ...
                return error(defines.Codes.NOT_FOUND, "Service with id ("+str(id)+") is not attributed for this device") 
               
            self.payload = self.get_payload()
            return status(defines.Codes.DELETED, self.payload)
        except:
            return error(defines.Codes.BAD_REQUEST, "Request query must specify an id of the service to remove from the device services")
